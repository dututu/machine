    ...
    public function requestopen()
    {
        ...
        if ($pos  ===  false) {
            ...
            if($machine['businessstatus'] == MachineBusinessStatusEnum::NORMAL && ($machine['status'] == MachineStatusEnum::UNOPENED || $machine['status']== MachineStatusEnum::CLOSED|| $machine['status']== MachineStatusEnum::ONLINED)){
                ...
                //展会临时添加 start
                $arr=array("XX2111804160007","XX2111804160048","XX2111804160045","XX2111804160029","XX2111804160034","XX2111804160021","XX2111804160027","XX2111804160043","XX2111804160050","XX2111804160035","XX2111804160009","XX2111804160039","XX2111804160008","XX2111804160032","XX2111804160024",
                    "XX2111804160041","XX2111804160011","XX2111804160028","XX2111804160016","XX2111804160047","XX2111804160030","XX2111804160026","XX2111804160005","XX2111804160036","XX2111804160017","XX2111804160020","XX2211805100001","XX2111804160014","XX2111804160038","XX2111804160004","XX2111804160010",
                    "XX2111804160012","XX2111804160019","XX2111804160023","XX2111804160040","XX2111804160018","XX2111804160006","XX2111804160046","XX2111804160042","XX2111804160003","XX2111804160049","XX2111804160015","XX2111804160033","XX2111804160031","XX2111804160037");
                if(in_array($machine['containerid'],$arr)){
                    $starttime = date("Y-m-d H:i:s");
                    $endtime = date("Y-m-d H:i:s",strtotime("-3 day"));
                    $countsql = 'SELECT COUNT(*) c FROM goodsorder a where  a.machineid ='.$machineid.' and a.userid='.$uid.' and a.createtime>\''.$endtime.'\' and a.createtime<\''.$starttime.'\'';
                    $records =  Db::query($countsql);
                    $records =(int)reset($records)['c'];
                    if($records>0){
                        return status(203, '您已经购买过一次');
                    }
                }
                //展会临时添加 end
                $code = 200;
        ...
        if(!$machine){
            ...
            if($machine['rfidtypecode'] == 3){//重力感应柜
                ...
                if(isset($masterresult['code'])){
                    ...
                    $traderesult = '';
                    for ($x=0; $x<5; $x++) {
                        $traderesult = $gboxApi->GetTransEvt($option2);
                        Log::info($traderesult);
                        if(isset($traderesult['code'])){
                            $code = $traderesult['code'];
                            if($traderesult['code'] == 0){
                                $code = 200;
                                break;
                            }else if($traderesult['code'] == 3){
                                //机柜离线
                                $code = 3;
                                break;
//                                $message = '机柜离线';
                            }else if($traderesult['code'] == 203){
                                //交易不存在
                                $code = 203;
//                                $message = '网络超时或交易不存在';
                            }else{
                    ...
                    if($code == 200){
                        ...
                        $machine['status'] = MachineStatusEnum::OPENED;
                        $machine->save();
                        return result(200, $traderesult['msg'], $data);
                    }else{
                        $order['orderstatus'] = GoodsOrderStatusEnum::CANCELED;
                        $order['doorstatus'] = GoodsOrderDoorStatusEnum::UNPENED;
                        $goodsModel->save($order);
                        $machine['status'] = MachineStatusEnum::UNOPENED;
                        $machine->save();
                        return result(201, $traderesult['msg'], $data);
                    }
                }
