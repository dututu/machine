...
class Testbox extends Controller   
{
    ...
    public function bind()
    {
        ...
        $result1 = Db::table('machine')->where('containerid', $containerid)
                        ->update(['boxdevid' => $devid, 'boxdevuid' => $devuid,'boxsn'=>$boxsn,'businessstatus' => 3,'status' => 10,'camsn'=>$csn]);
    ...
    */
    ...
    public function queryWifi(){
        if( Request()->isPost()){
            ...
            $option = [];
            $option['dev_uid'] = $devuid;
            $masterresult = $gboxApi->QueryWifiDev($option);
            return json($masterresult);
        }else{
            $data['code'] = 1004;
            $data['msg'] = '请求失败';
    ...
    public function query4G(){
        if( Request()->isPost()){
            ...
            $option = [];
            $option['dev_uid'] = $devuid;
            $masterresult = $gboxApi->QueryCsq($option);
            return json($masterresult);
        }else{
            $data['code'] = 1004;
            $data['msg'] = '请求失败';
